...
int expand_alias(arg_inventory_t *arginv)
{
	...
	if (node != NULL)
	{
		tokenize(&cmd_tokens, node->command);						count = 0;
		...
		for (i = 0; i < cmd_tokens.tokensN; i++)
			commands[i] = _strdup((char *)cmd_tokens.tokens[i].str);		free(arginv->commands[0]);
		free(arginv->commands);
		commands[count + cmd_tokens.tokensN - 1] = NULL;				count = cmd_tokens.tokensN;							arginv->commands = commands;							delete_tokens(&cmd_tokens);
		return (count - 1);
	}
	return (0);
}
